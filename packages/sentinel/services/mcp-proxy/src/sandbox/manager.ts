...
export class ShadowManager {
  ...
  setPendingReview(timelineId: string): void {
    this.timelineManager.updateStatus(timelineId, 'pending_review');
  }

  async approvePending(timelineId: string): Promise<DiffResult> {
    return this.merge(timelineId);
  }

  async rejectPending(timelineId: string): Promise<void> {
    return this.abandon(timelineId);
  }

  async cleanupAll(): Promise<void> {
    for (const [id, workspace] of this.activeWorkspaces) {
      const timeline = this.timelineManager.getTimeline(id);
      if (timeline && (timeline.status === 'merged' || timeline.status === 'abandoned')) {
