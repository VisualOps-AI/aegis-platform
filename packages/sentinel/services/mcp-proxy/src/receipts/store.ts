...
export class EventStore {
  ...
  getPendingTimelines(): Record<string, unknown>[] {
    const stmt = this.db.prepare(
      "SELECT * FROM timelines WHERE status = 'pending_review' ORDER BY created_at DESC"
    );
    return stmt.all() as unknown as Record<string, unknown>[];
  }

  storeDiffResult(timelineId: string, diff: unknown): void {
    this.insertTimelineEvent({
      timelineId,
      eventType: 'diff_computed',
      timestamp: new Date().toISOString(),
      dataJson: JSON.stringify(diff),
    });
  }

  getDiffResult(timelineId: string): unknown | null {
    const stmt = this.db.prepare(
      "SELECT data_json FROM timeline_events WHERE timeline_id = ? AND event_type = 'diff_computed' ORDER BY id DESC LIMIT 1"
    );
    const rows = stmt.all(timelineId) as unknown as { data_json: string | null }[];
    if (rows.length === 0 || !rows[0].data_json) return null;
